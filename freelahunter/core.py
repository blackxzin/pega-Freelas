...
class ProfileService:
    ...
    def load(self):
        p=Profile(name='FreelaHunter User', headline='Desenvolvedor Full Stack / estudante de ADS', skills=['Python','Java','JavaScript','TypeScript','HTML','CSS','React','Next.js','FastAPI','APIs REST','SQL','Linux','Git','GitHub','Automação','Integrações','IA'])
...
class ProposalGenerator:
    def generate(self, job, analysis, profile, projects, price=None):
        ...
        msg=f'Olá! Vi que o projeto busca {job.title.lower()} e que o resultado esperado envolve uma entrega funcional, documentada e fácil de manter. Posso abordar o escopo com {tech}, estruturando a implementação em etapas, validando os requisitos da API e deixando testes para os fluxos críticos. Tenho experiência comprovável em {project}, relacionado a este tipo de entrega, e trabalho com versionamento Git, integração de APIs e comunicação clara durante o desenvolvimento. Primeiro confirmo o fluxo principal, entradas, saídas e critérios de aceite; em seguida proponho uma divisão curta de tarefas para reduzir riscos e permitir validação incremental. Para estimar com precisão: quais integrações, ambiente de hospedagem e critérios de aceite são prioritários? Posso compartilhar um plano inicial, alinhar o cronograma e começar pela parte de maior valor para o projeto.'
...
class Database:
    ...
    def within_limit(self, bucket, maximum):
        row=self.conn.execute('SELECT count FROM limits WHERE bucket=?',(bucket,)).fetchone(); return not row or row[0] < maximum
    def consume_limit(self, bucket):
        self.conn.execute('INSERT INTO limits(bucket,count,reset_at) VALUES(?,?,?) ON CONFLICT(bucket) DO UPDATE SET count=count+1',(bucket,1,now())); self.conn.execute('INSERT INTO limit_events(bucket,created_at) VALUES(?,?)',(bucket,time.time())); self.conn.commit()
...
def run_pipeline(provider, db, profile=None, auto_send=False, dry_run=True, kill_switch=True, sender=None):
    profile=profile or ProfileService().load(); portfolio=PortfolioService(); ai=AIService(); gen=ProposalGenerator(); val=ProposalValidator(); sender=sender or MockSender(); policy=SendPolicyEngine(auto_send,dry_run,kill_switch); stats={'found':0,'new':0,'duplicates':0,'filtered':0,'analyzed':0,'proposals':0,'review':0,'sent':0}
    for job in provider.search():
        stats['found']+=1; job.platform=provider.name; job,dup=db.upsert_job(job)
        if dup: stats['duplicates']+=1; continue
        stats['new']+=1
        if not FilterService().accepts(job): job.status=JobStatus.REJECTED; continue
        stats['filtered']+=1; projects=portfolio.relevant(job,profile); analysis=ai.analyze(job,profile,projects); stats['analyzed']+=1; price,_=PriceEstimator(hourly_rate=100,minimum_project_price=200).estimate((analysis.estimated_hours_min+analysis.estimated_hours_max)//2, budget_max=job.budget_max); p=gen.generate(job,analysis,profile,projects,price); val.validate(p,profile); stats['proposals']+=1; key=sha(provider.name+job.external_id+profile.id); db.save_proposal(p,key)
        decision=policy.decide(provider,analysis,p,sender,db.already_sent(key));
        if decision=='AUTO_SEND':
            if dry_run: stats['review']+=1
            else: db.mark_sent(key,sender.send(p)['external_id']); stats['sent']+=1
        else: stats['review']+=1
    return stats
