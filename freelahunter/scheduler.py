    ...
    def run_cycle(self):
        ...
        try:
            for provider in self.registry.enabled():
                ...
                try: result[provider.name]=run_pipeline(provider,self.db,auto_send=self.config.auto_send,dry_run=self.config.dry_run,kill_switch=self.config.kill_switch)
