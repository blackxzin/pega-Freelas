...
def main():
    ...
    db=Database(':memory:'); s=run_pipeline(AuthorizedMockProvider(jobs),db,auto_send=True,dry_run=False,kill_switch=False,sender=MockSender()); print(f"Jobs: {s['found']}\nFiltered: {s['filtered']}\nAnalyzed: {s['analyzed']}\nProposals: {s['proposals']}\nAuto-send eligible: {s['sent']}\nSent: {s['sent']}\nDuplicates prevented: {s['duplicates']}\nErrors: 0")
